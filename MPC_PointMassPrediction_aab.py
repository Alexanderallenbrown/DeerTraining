...
#         return J
    ...
    def calcOptimal(self,carnow, deernow,yroad):
        ...
        #now use the optimal path to generate
        L = carnow.x[2]*self.dtp*self.Np #this is the preview lever arm
        #preview is yv+L*sin(psi)
        yp = carnow.x[0]+L*carnow.x[4]
        #error is the path position minus the preview y position
        e = self.opt_path_y[-1]-yp
        #optimal steering is then Kp*e
        #opt_steering = self.Kpsteer*e
...
def demo_CVdeer():
    ...
    weight = 10.0
    MPC = MPC_PM(q_lane_error = 10*weight,q_obstacle_error =0.1/weight*10,q_lateral_velocity=0.00,q_steering_effort=0.00,q_accel = 0.00,predictionmethod='CV')
