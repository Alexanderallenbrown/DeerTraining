...
class MPC_G:
    def __init__(self, Np=5, dtp=.2, q_obstacle_error = 1.0, q_cruise_speed = 1.0, q_x_accel=1.0, gas_max=1.0, brake_max = 1, epsilon = 0.0001,downsample_horizon = 'false',predictionmethod = 'static'):
        self.Np = Np
        self.dtp = dtp
    ...
    def predictCar(self,carnow,x_accelvector):
        ...
        for k in range(0,self.Np):
            if x_accelvector_upsampled[k] > 0:
                ...
                brake = 0
                xcar_pred[k,:],xdotcar_pred[k,:],steer = predictCar.heuns_update(gas = gas, brake = brake, steer = 0, cruise = 'off')
            else:
                gas = 0
...
def demo_GAdeer():
    ...
    for k in range(1,len(t)):
            ...
            if ((sqrt((deer.x_Deer - car.x[2])**2+(deer.y_Deer - car.x[0])**2) < x_acceldistance) and (deer.x_Deer>car.x[2])):
                ...
                carx[k,:],carxdot[k,:],steer = car.heuns_update(gas = gas, brake = brake, steer = 0, cruise = 'off')
                ...
                carx[k,:],carxdot[k,:],steer = car.heuns_update(steer = 0, setspeed = setSpeed,)
