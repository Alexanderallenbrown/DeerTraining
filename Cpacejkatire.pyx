...
cdef float pow(number,power):
    return number**power

cdef class PacejkaTire:
    cdef float K,alpha,camb,Fz,fric_x_max,fric_y_max,C,D,C_alpha,E,Sh,Sv,delB,B,phi,Fy,Fy_comb,Fx,Bx,BCDx,Ex
    def __init__(self,iFz=5000,ifric_x_max=1.1,ifric_y_max=1.0):
        ...
        self.fyparms = array([-22.1, 1011, 1078, 1.82, .208, -0.00, -0.354, .707, .028, 0.0, 14.8, .022, 0.0],dtype=float);
        self.mzparms = array([-2.72, -2.28, -1.86, -2.73, 0.110, -0.07, .643, -4.04, 0.015, -0.066, .945, 0.03, 0.07],dtype=float);
        self.fxparms = array([-21.3, 1144, 49.6, 226, 0.069, -0.006, 0.056, .486],dtype=float);
    ...
    def findKappa(self, iFz, ialpha, iFx):
        kappa = linspace(-2,2,100,dtype=float)
        Fx = zeros(100,dtype=float)
        cdef Py_ssize_t ind
    ...
    def calcFy(self,iFz, ialpha, ikappa, icamb):
        ...
        cdef float kappac,alphac,alphastar,beta,muy_now,mux_max,muy
    ...
    def calcFx(self,iFz, ikappa, ialpha):
        ...
        #print self.Fx
        cdef float kappac,alphac,alphastar,thecosine,beta,mux_now,muy_max,mux
